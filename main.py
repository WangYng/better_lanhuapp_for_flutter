...
# 检测文件名是否包含中文字符,
from env import download_dir, project_image_dir, project_code_file, isForceCover
...
if __name__ == '__main__':
    for root, dirs, files in os.walk(download_dir):
        ...
        for f in files:
            ...
            # 过滤条件: 后缀是zip, zip文件名不包含中文, zip包含ios对应的三种规格图片文件
            if path.endswith('.zip') and not contain_chinese(f) and is_lanhu_zip(path):
                # 项目中没有导入过这个图片
                if not exists_in_project(path):
